...
TanStack DB provides a powerful mutation system that enables optimistic updates with automatic state management. Mutations support instant local updates that are superseded by server writes, creating a responsive user experience while maintaining data consistency.

The mutation system is built around a pattern of **optimistic mutation → backend persistence → sync back → confirmed state**. Local changes are applied immediately as optimistic state, then persisted to your backend, and finally the optimistic state is replaced by the confirmed server state once it syncs back.
