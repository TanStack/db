...
- `NaN` and invalid Dates now have a well-defined sort position (alongside nulls) instead of comparing equal to every value. This fixes non-deterministic ordering when sorting by a field that contains such values, and — because the index ordering is no longer corrupted by them — range queries on a field that contains a `NaN` or invalid Date are once again served by the index (the non-matching value is removed by the existing re-filter) rather than dropping genuinely matching rows.
