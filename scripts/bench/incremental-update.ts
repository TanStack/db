...
} from '../../packages/db/src/query/live/perf.js'
import type {
  ChangeMessageOrDeleteKeyMessage,
  Collection,
  SyncConfig,
} from '../../packages/db/src/index.js'
import type { PerfTraceSnapshot } from '../../packages/db/src/query/live/perf.js'
...
const report = {
  ...
  results,
  overhead: computeOverhead(results),
}

mkdirSync(options.outDir, { recursive: true })
const outputPath = join(options.outDir, `incremental-update-${Date.now()}.json`)
...
async function runCase(
  ...
  for (let i = 0; i < options.warmup; i++) {
    ...
    await flushMicrotasks()
    await cleanupWrite(writeResult, tracing)
  }
  ...
  const trace = getPerfTraceSnapshot()
...
function computeOverhead(results: Array<RunResult>) {
  ...
  for (const disabled of results.filter((result) => !result.tracing)) {
    ...
    overhead.push({
      ...
      p95OverheadMs: enabled.writeSummary.p95Ms - disabled.writeSummary.p95Ms,
    })
  }
...
function formatTextReport(
  report: {
    ...
    overhead: ReturnType<typeof computeOverhead>
  },
  outputPath: string,
): string {
...
function hashMetrics(trace: PerfTraceSnapshot, limit: number) {
  return trace.metrics
    .filter((metric) => metric.name.startsWith(`hash.`))
    .sort((a, b) => b.total - a.total || b.calls - a.calls)
    .slice(0, limit)
}
