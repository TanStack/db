...
const phase=process.env.EVIDENCE_PHASE??'green',response=await fetch('http://127.0.0.1:4179/src/endpoint.ts'),text=await response.text()
...
await writeFile(`evidence/phase-2/${phase}-dev-delivery.json`,JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2))
