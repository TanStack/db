...
await writeFile(`evidence/phase-2/${phase}-maps-build.log`,built.stdout+built.stderr)
...
const code=await readFile('src/endpoint.ts','utf8'),correspondence=[]
function position(source,index){const lines=source.slice(0,index).split('\n');return {line:lines.length,column:lines.at(-1).length}}
for(const [directory,token] of [['dist/server','SERVER_ONLY_SENTINEL'],['dist/client','optimistic:']]){
 ...
 for(const path of paths.filter(p=>p.endsWith('.js'))){const source=await readFile(path,'utf8'),index=source.indexOf(token);if(index<0)continue
  ...
  assert.ok(actual.source.split('?')[0].endsWith('/src/endpoint.ts'),JSON.stringify(actual));assert.equal(actual.line,expected.line)
...
await writeFile(`evidence/phase-2/${phase}-built-maps.json`,JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2))
