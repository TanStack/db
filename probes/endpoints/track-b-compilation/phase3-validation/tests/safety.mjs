...
const original=await readFile('src/endpoint.tsx','utf8'), route=await readFile('src/routes/index.tsx','utf8')
...
try{
 ...
 for(const [name,source,page,marker,sharedSource] of [
  ['retained-helper',original+'\nexport const retained = database()\n',route.replace('{Probe}','{Probe,retained}').replace('<Probe/>','<><Probe/><output>{retained}</output></>'),'DATABASE_ONLY_SENTINEL'],
  ...
  await writeFile('src/shared.ts',sharedSource??shared);await writeFile('src/endpoint.tsx',source);await writeFile('src/routes/index.tsx',page)
  const built=spawnSync(process.execPath,['node_modules/vite/bin/vite.js','build'],{encoding:'utf8'})
  await writeFile(`evidence/${phase}-${name}.log`,built.stdout+built.stderr)
...
}finally{await rm('src/probe-secret.server.ts',{force:true});await writeFile('src/shared.ts',shared);await writeFile('src/endpoint.tsx',original);await writeFile('src/routes/index.tsx',route);await writeFile(`evidence/${phase}-safety.json`,JSON.stringify(results,null,2))}
