...
const source=await readFile('src/endpoint.tsx','utf8'),phase=process.env.EVIDENCE_PHASE??'green',results=[]
try{
 for(const [name,changed] of [['extra-property',source.replace(' input:z.object',' extra:true,\n input:z.object')],['syntax-error',source.replace(' input:z.object',' input:??z.object')]]){
  await writeFile('src/endpoint.tsx',changed)
  await new Promise(resolve=>setTimeout(resolve,300))
  const response=await fetch('http://127.0.0.1:4291/src/endpoint.tsx'),body=await response.text()
...
}finally{await writeFile('src/endpoint.tsx',source);await new Promise(resolve=>setTimeout(resolve,500));await writeFile(`evidence/${phase}-grammar-error.json`,JSON.stringify(results,null,2))}
