...
try{
 record('source',{sha256:createHash('sha256').update(original['src/endpoint.ts']).digest('hex'),declaration:'addTodo',generated:'addTodoRpc',location:'src/endpoint.ts:5-12',sourceMap:false})
 ...
 await rm('dist',{recursive:true,force:true});await writeFile('evidence/modified-build.log',build());const client=await chunks('dist/client'),server=await chunks('dist/server');assert.equal(client.includes('SENTINEL'),false);assert.ok(server.includes('helper-v2'));assert.equal(server.includes('handler-v1'),false);assert.equal(server.includes('addTodoRpc'),false);record('clean modified build',{status:'pass',clientSentinels:false,serverRequired:true,staleName:false})
 // Counterexamples deliberately keep imports live outside handler.
 await writeFile('src/endpoint.ts',original['src/endpoint.ts']+'\nexport const retained = database()\n');await writeFile('src/routes/index.tsx',original['src/routes/index.tsx'].replace("{addTodo}","{addTodo,retained}").replace('<main>','<main>{retained}'))
 await writeFile('evidence/retained-build.log',build());assert.ok((await chunks('dist/client')).includes('DATABASE_ONLY_SENTINEL'));record('retained helper exclusion',{status:'fail',expectedCounterexample:true,leaked:'DATABASE_ONLY_SENTINEL'})
 await writeFile('src/endpoint.ts',"import './server-side-effect'\n"+original['src/endpoint.ts']);await writeFile('src/routes/index.tsx',original['src/routes/index.tsx']);await writeFile('evidence/side-effect-build.log',build());assert.ok((await chunks('dist/client')).includes('SIDE_EFFECT_ONLY_SENTINEL'));record('side-effect import exclusion',{status:'fail',expectedCounterexample:true,leaked:'SIDE_EFFECT_ONLY_SENTINEL'})
 await writeFile('src/endpoint.ts','export const removed = true\n');await writeFile('src/routes/index.tsx',"import {createFileRoute} from '@tanstack/react-router'\nexport const Route=createFileRoute('/')({component:()=> <p>Endpoint removed</p>})\n");await page.waitForTimeout(800);await page.goto('http://127.0.0.1:4179');assert.ok((await page.locator('body').innerText()).includes('Endpoint removed'));await replay('old RPC after delete');await rm('dist',{recursive:true,force:true});await writeFile('evidence/deleted-build.log',build());assert.equal((await chunks('dist/server')).includes('SERVER_ONLY_SENTINEL'),false);record('delete declaration dev and clean build',{status:'pass',staleServerImplementation:false})
}finally{for(const [f,s] of Object.entries(original))await writeFile(f,s);await browser.close();await writeFile('evidence/results.json',JSON.stringify(results,null,2))}
await writeFile('evidence/build.log',build())
