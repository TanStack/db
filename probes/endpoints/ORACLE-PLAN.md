...
Current browser instrumentation compares complete collection rows and the
consumer's rendered projection (ordered IDs and text). Consumer checks occur
after two animation frames at named checkpoints; they do not establish equality
at every intervening React commit or for columns the fixture does not render.
The concurrent invocation snapshot is compared as captured, before any later
observation can replace it. See the [guide-driven repairs and receipts](integrated-todo/ORACLE-REPAIR-RESULTS.md).
...
Only the outer scenario decides whether a shrink preserves the first failure.
Nested sequence runners propagate it immediately. Save the original and reduced
inputs, semantic law, checkpoint and distinguishing location; reject a shrink
that changes the accusation into a different mismatch or infrastructure failure.
Replay reports whether the same violation occurred. Legacy inputs lacking a
failure identity earn execution evidence only.

Freeze assertion evidence before cleanup. Attempt all releases, retain secondary
cleanup failures separately, and write the final verdict afterward. Capture code
hashes and dependency versions before executing a campaign. Named comparison
counts and fault-site records distinguish reaching a check from merely requesting
a mutant. Surviving and unreached fault controls must not report success.
