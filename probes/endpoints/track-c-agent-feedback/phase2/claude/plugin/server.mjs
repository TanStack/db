...
const traceFile = new URL('../../traces/claude-lsp-wire.jsonl', import.meta.url)
const marker = 'C2_NATIVE_LSP_4c671efb'
...
function publish(uri, text, version) {
  ...
  send({ method: 'textDocument/publishDiagnostics', params: { uri, version, diagnostics: text.includes("'id'") ? [] : [{ range: { start: { line: 0, character: 14 }, end: { line: 0, character: 27 } }, severity: 1, code: 'ENDPOINT_ORDER_NOT_TOTAL', source: 'synthetic-endpoints-native-probe', message: `${marker}: Synthetic fixture order lacks an id tie-breaker. This is not database analysis. Source revision ${sha256}`, data: { sha256, synthetic: true } }] } })
}
function receive(message) {
  trace('client-to-server', message)
  const p = message.params
  if (message.method === 'initialize') send({ id: message.id, result: { capabilities: { textDocumentSync: { openClose: true, change: 1, save: { includeText: true } } }, serverInfo: { name: 'synthetic-endpoints-native-probe', version: '0.2.0' } } })
