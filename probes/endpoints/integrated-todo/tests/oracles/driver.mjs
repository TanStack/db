...
export class Driver {
  constructor(adapter = {}) {
    ...
    this.evidence = adapter.evidence ?? new Evidence('endpoints.browser')
  }
  buildCache = new Set()
  ...
  async init() {
    ...
    await writeFile(
      join(this.dir, 'src/oracle-probe.ts'),
      process.env.ENDPOINT_ORACLE_TEST_FAULT === 'immediate-snapshot'
        ? replaceOnce(
            browserProbe,
            'rows:snapshot()}\n   },outcomes',
            'rows:snapshot().map(rows=>rows.map(row=>({...row,text:row.text+"__wrong_immediate__"})))}\n   },outcomes',
          )
        : browserProbe,
    )
    {
    ...
    if (process.env.ENDPOINT_ORACLE_STRATEGY === 'client-refetch') {
      ...
      source = replaceOnce(source, 'reads: requests', 'reads: []')
      source = replaceOnce(
        source,
  ...
  async prepare(program) {
    this.evidence.phase = 'build'
    let source = (this.adapter.renderProgram ?? renderProgram)(program)
    if (process.env.ENDPOINT_ORACLE_TEST_FAULT === 'rendered-text') {
      assert.ok(
        source.includes('{row.text}'),
    ...
    if (this.currentKey !== key) {
      ...
      if (schemaSnapshot) {
        ...
        await writeFile(
          join(this.dir, '.endpoints/schema.json'),
  ...
  async checkpoint(page, program, label, immediate) {
    ...
    if (
      ...
      this.evidence.fault('immediate-snapshot', { label })
    this.evidence.check('collection-rows', actual, expected, { checkpoint })
    ...
    const consumers = await page
      ...
      .evaluateAll((sections) =>
        sections.map((section) =>
          [...section.querySelectorAll('[data-row]')].map((row) => ({
            id: row.dataset.row,
    ...
    this.evidence.check(
      ...
      expected.flatMap((rows, i) =>
        program.unsubscribed?.includes(i) || program.gced?.includes(i)
          ? []
          : [rows.map((row) => ({ id: row.id, text: row.text }))],
      ),
      { checkpoint },
    )
  }
  async run(program, steps) {
    return this.evidence.run({ program, sequences: [steps] }, () =>
      this.runSequence(program, steps),
    )
  }
    ...
    try {
      ...
      page.on('response', (response) => {
        if (
          ...
          const work = response.body().then((body) =>
            wire.push({
      ...
      if (armed) {
        await this.evidence.cleanup('write-gate', () =>
          this.control({
            command: 'write',
            token: armed,
            reject: true,
        ...
        await this.evidence.cleanup('read-gate', () =>
          this.control({ command: 'read' }),
        )
      }
  ...
  async close() {
    await this.evidence.cleanup('browser', () => this.browser?.close())
    await this.evidence.cleanup('server', () => stop(this.server))
    await this.evidence.cleanup('reference', () => this.reference?.close())
    if (this.dir)
      await this.evidence.cleanup('directory', () =>
