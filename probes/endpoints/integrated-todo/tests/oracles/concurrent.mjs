...
evidence.name = 'endpoints.concurrent'
const report = {
...
async function run(program, sequence) {
  ...
  try {
    ...
    await page.route('**/*', (route) => {
      ...
      routeWork.push(work)
      work.catch(() => {}) // Retained and checked below; prevent an unhandled rejection.
      return work
    })
    let folded
    async function checkpoint(label, immediate) {
      ...
      await driver.checkpoint(page, program, label, immediate)
    ...
    for (const token of heldResponses.keys())
      await evidence.cleanup('write-gate:' + token, () =>
        driver.control({ command: 'write', token }),
      )
    await evidence.cleanup('read-gate', () =>
      driver.control({ command: 'read' }),
    )
    await evidence.settled('routes', routeWork)
    await evidence.cleanup('routes', () =>
      page?.unrouteAll({ behavior: 'ignoreErrors' }),
    )
    await evidence.cleanup('browser-context', () => context.close())
  }
}
