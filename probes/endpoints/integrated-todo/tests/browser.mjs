...
try{
 ...
 const failedRead=crypto.randomUUID();await start(failedRead,'Committed, then read retried')
 await until(async()=>(await control()).rows.some(x=>x.id===failedRead))
 const retrying=await state();assert.equal(retrying.state,'persisting');assert.equal(retrying.rows.some(x=>x.id===failedRead),true)
 await page.waitForFunction(()=>window.outcome==='resolved')
 const recovered=await state();assert.equal(recovered.state,'completed');assert.equal(recovered.rows.some(x=>x.id===failedRead),true)
 evidence.checks.push({name:'post-write refetch failure retries without rolling back the optimistic row',retrying,recovered})
