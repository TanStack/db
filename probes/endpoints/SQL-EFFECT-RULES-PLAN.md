...
Status: initial rule kernel and PostgreSQL oracle implemented; compiler rollout
and broad SQL binding remain open. See [execution receipts and limits](integrated-todo/SQL-EFFECT-RULES-RESULTS.md).
The design is grounded by the
...
The instruments completed on 2026-09-14. The initial implementation exercises
build-time facts and body-derived effects through the real registry and client
collections; existing compiler consumers still use the legacy analyzer.
The Kitchen port
...
supply expected dependency relationships. Save the original and reduced traces,
law, checkpoint, first differing collection, operation family, source hashes,
versions, seed and shrink path. Accept a shrink only if it preserves that failure
identity; classify setup errors, progress timeouts, semantic rejection and
survival separately. Replay must report whether the same violation occurred.

Keep expected admitted baselines independent of the client, including baselines
retained across unrelated external writes. Capture same-turn rows before any
await. Record each named comparison and fault's path reach. Freeze failure
evidence before cleanup; retain cleanup errors separately and attempt every
release. Browser collection, notification and rendered-value observations earn
separate coverage credit.

The [fresh guide loss audit](design/field-trip-optimistic-coherence/oracle-guide-loss-audit/AUDIT.md)
identifies which older runners still fall short of these requirements. The new
runner does not retroactively repair or expand their coverage.
