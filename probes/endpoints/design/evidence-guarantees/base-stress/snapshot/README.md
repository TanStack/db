...
This is the first implementation experiment following the
[exploratory Design Grammar](../endpoints/design/evidence-guarantees/base-grammar/README.md).
Implementation choices below are separate from that frozen result.
...
reasons. No severity, admission policy, aggregate confidence score, automatic
...
- Replay compares the full supplied case data and claim identity. Packages must
  ...
  neither. Fresh passing evidence does not resolve a challenge implicitly.
...
Ten tests pass. The argument oracle compares backward evaluation with a separate
...
Four actual mutations of temporary kernel copies are detected by the unchanged
oracles: accepting one conjunct, circular self-support, ignoring freshness, and
dropping failure reports. Initially the freshness fault escaped because low bits
...
then failed at the expected assertions; the unchanged kernel passed.
