...
export class EvidenceBase {
  ...
  resolve(challengeId: number, replayId: number): void {
    ...
    if (
      ...
      !failed ||
      !replay ||
      challenge.resolution !== undefined ||
      !replay.passed ||
  ...
  #assess(claim: Claim, ancestors: Set<string>): Assessment {
    ...
    const failures = this.#challenges.filter((challenge) => {
      ...
      // Conservative prototype: same logical claim remains challenged across
      // context changes until replay. No automatic cross-version dismissal.
      return (
        challenge.resolution === undefined &&
    ...
    for (const argument of candidates) {
      ...
      if (!rule) {
        result.reasons.push(`Unregistered rule: ${argument.rule}`)
        continue
      }
      ...
      for (const id of argument.observations) {
        ...
        if (
          ...
          rejection = `Missing, stale or failing observation: ${id}`
          break
        }
        observations.push(observation)
      }
      if (rejection) {
        result.reasons.push(rejection)
        continue
      }
      try {
      ...
      if (children.every((child) => child.status === 'supported')) {
        return {
          ...result,
          status: 'supported',
          reasons: [],
          ...
          observations: [
            ...new Set([
              ...
              ...children.flatMap((c) => c.observations),
            ]),
          ],
          rules: [
            ...new Set([rule.id, ...children.flatMap((child) => child.rules)]),
          ],
        }
      }
      // A contradicted premise defeats this route, not an independent route.
      result.reasons.push(...children.flatMap((child) => child.reasons))
      result.gaps.push(
        ...children
    ...
    result.gaps = distinctClaims(result.gaps.length ? result.gaps : [claim])
    return result
  }
}
