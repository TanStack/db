...
An agent can produce an implementation quickly. Somebody still has to say
...
This RFC proposes **TanStack Trust**, a small evidence-mechanics layer for
building those systems inside specific software domains. TanStack Trust does
not decide whether an agent, change, or application is trustworthy in the abstract. A domain
...
For the foreseeable future, Trust will be built and shipped as part of TanStack
DB Endpoints. It is an internal architectural layer and evidence contract, not a
separate product or package roadmap. “Small” describes the authority the layer
is allowed to exercise, not a promise about its eventual line count or
operational footprint. Keeping Trust distinct from Endpoints domain semantics
means maintaining clean ownership, module, test, status, and interface seams
inside one product. Extraction remains an optional later decision.

The current Endpoints worktree implements a finite local Trust kernel, verified
file storage, one Endpoints check, and CLI, LSP, and MCP adapters over one
service. That is a useful vertical slice. It is not evidence that the
architecture spans every software domain, that its registered rules are sound,
or that agent-produced software is now safe to ship.
...
The adjacent [AIUC argument](https://www.latent.space/p/aiuc) is useful here. It
separates raw capability from deployability, emphasizes bounded promises over
unlimited guarantees, and asks
...
The first domain is Endpoints, and it will remain the product home for Trust
while this architecture develops. The near-term question is whether these
mechanics make Endpoints claims more inspectable and maintainable for agents and
humans. It is not whether Trust can already justify a standalone product.
...
This is an authority map, not a package map. The five layers may share one
repository, release, process, and user-facing product. The separation matters
because it prevents Endpoints domain rules, generic evidence bookkeeping, and
runtime policy from silently authorizing one another. It does not require an
independent `@tanstack/trust` package.
...
**Status:** This authority map is a **proposed contract**. The local Endpoints
prototype implements much of the Trust column and keeps consumer policy out. It
does not yet expose the stable internal extension boundary that a future
...
application failure from check failure. A checker execution can succeed while
producing a `fail` finding because it correctly observed an application
violation; a checker that never reached the application has produced no
correctness observation at all.
...
**Status:** One Endpoints-owned package instantiates a `CheckContract` and a
registered rule. Generic internal package authoring, rule inspection and
registration, rubric admission, and formal-check integration are **absent**.
They can first be developed for Endpoints without committing to a standalone
Trust distribution.
...
fails outside evidence history rather than manufacturing an observation. The
failure can remain in CI or operational logs, but that separate history is not
specified by the current Trust model. **Implemented evidence boundary; separate
operational history unresolved.**
...
select histories that can reach it, assign fixture responsibility, and observe
the production checkpoint. It also requires authors to distinguish harness
failure from application findings, preserve the original violation while
reducing a trace, and retain promises the bounded check did not establish.
...
### The Endpoints agent interface still missing

For agents to construct and maintain Endpoints guarantees rather than merely
query one fixed check, the operation layer needs at least:
...
These operations can remain Endpoints-owned while their evidence semantics stay
cleanly separated. Adding them is more important than extracting a package or
adding another transport. It is the difference between “an agent can call the
demo” and “an agent can participate in Endpoints' evidence lifecycle.”
...
## 8. Endpoints is the incubation host and bounded reconstruction

TanStack DB Endpoints supplies the first production-shaped domain package:
`endpoints/safe-skip-refetch@1`.

It is also where Trust will live for now. Endpoints owns the complete product
experience: its domain rules and checks, the internal Trust evidence mechanics,
the agent-facing interfaces, and the consumer policy that will eventually act
on assessments. Keeping those responsibilities separate inside Endpoints makes
their contracts inspectable without forcing premature package extraction.
...
the base spans other domains. That limitation remains visible, but it does not
create a near-term requirement to build another implementation.
...
The source trail leaves these boundaries unresolved:
...
- A stable internal domain-package manifest and extension API that could support
  later extraction without requiring it now.
...
- The conditions, ownership needs, or repeated friction that would justify
  extracting Trust from Endpoints into its own release boundary.
...
- Independent range remains unmeasured; no second domain implementation is
  currently planned. A future extraction decision may create a reason to test
  another domain then.
...
The recorded gaps suggest an Endpoints-first sequence:

1. **Complete the Endpoints trust loop.** Connect Endpoints rules and checks,
   internal Trust evidence mechanics, consumer policy, and agent-facing use in
   one end-to-end product path.
2. **Keep the internal seams explicit.** Decide applicability's ownership and
   result shape; define typed claims, observations, rules, arguments, challenges,
   dependencies, and check contracts without requiring a separate package.
3. **Make explanation complete.** Expose routes, joint premises, alternatives,
   challenge effects, applicability decisions, check bounds, and shared
   dependencies through one stable Endpoints service result.
4. **Add the operations Endpoints agents need.** Internal package discovery,
   authorized rule registration, check execution, argument proposal, evidence
   ingestion, and gap/dependent-use queries should precede more transport work.
5. **Broaden and connect the real vertical.** Keep `safe-skip-refetch` bounded,
   expand analyzer evidence, scope dependencies by endpoint or artifact, and
   connect assessments to explicit Endpoints consumer policy without moving
   that policy into Trust.
6. **Ship and evaluate the agent experience.** Package the CLI, LSP, and MCP
   integrations; test actual editors and agents; and measure the full
   design/maintain/repair loop rather than adapter parity alone.
7. **Harden for the deployment Endpoints actually needs.** Add multi-process
   coordination, signatures, hostile-process isolation, remote execution, and
   scale work under explicit threat and deployment models.
8. **Revisit extraction only when its value is concrete.** Another use, release
   boundary, ownership need, or repeated friction may justify pulling Trust out.
   Until then, clean internal seams preserve the option without making it a
   milestone.

This ordering is proposed, not frozen product policy. Its governing principle
is simple: extraction or another interface is not progress if Endpoints agents
still cannot see or preserve the semantic obligations behind the result.
...
TanStack Trust gives that opportunity a specific architecture inside Endpoints.
Endpoints domain code owns the promise. The Trust layer owns evidence mechanics.
Workflows carry maintenance and repair obligations. Interfaces make one service
available to humans and agents. Endpoints consumer policy owns permission and
fallback. These can be separate responsibilities without being separate
products.

The Endpoints-hosted system succeeds when a later agent can tell, without
reconstructing the whole project from scratch:
