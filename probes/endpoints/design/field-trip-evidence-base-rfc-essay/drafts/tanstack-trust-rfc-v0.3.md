...
Cheaper generation also increases the number of candidate implementations.
Each candidate brings more claims to evaluate, checks to run, exceptions to
understand, and repairs that must remain legible after the session that made
them. The durable value of developer tooling therefore comes from shared
implementations that make application behavior more reliable, efficient, and
maintainable, and from the responsibility they safely remove while leaving the
remaining obligations visible.
...
> **Illustrative TanStack Router case—not implementation evidence:** Router
> automatically generates a file route tree with IDs, paths, and parent
> relationships. A domain package might claim that these generated relations
> are the ones the runtime uses to match URLs for a declared set of file-routing
> forms. One route could require both a
> generator observation and a runtime matching observation; another might use
> a separately authorized model check. The routes are alternatives, while the
> two observations in the first route remain joint work.

This is a hypothetical Trust application built on a concrete Router surface.
The [route-tree documentation](https://tanstack.com/router/latest/docs/routing/route-trees)
describes the tree as the structure used to match a URL to a component tree,
and the [file-based quick start](https://tanstack.com/router/latest/docs/framework/react/examples/quickstart-file-based)
imports an automatically generated `routeTree.gen` into `createRouter`. The
example does not establish that Trust already supports Router.
...
> **Illustrative TanStack Query case—not implementation evidence:** Query's
> [persistence contract](https://tanstack.com/query/latest/docs/framework/react/plugins/persistQueryClient)
> includes a timestamp and maximum-age window, buster, hydration options,
> storage representation, and cache settings such as `gcTime`. A package might
> define when evidence about restoring a persisted client applies across those
> inputs. Trust could retain the dependencies and the package's applicability
> result; it would not decide what counts as equivalent cache behavior.
...
The architecture becomes useful to agents when they can participate in the
full lifecycle of a guarantee. Calling one fixed check is not enough. Agents need
...
- a satisfying case, a violating case, and an unresolved case, plus who owns
  the fixtures that make those cases reachable;
...
- trust assumptions, unsupported constructs, known omissions, and open
  decisions;
...
> **Illustrative TanStack Table case—not implementation evidence:** Table
> exposes client-side [sorting](https://tanstack.com/table/latest/docs/framework/react/guide/sorting)
> and [filtering](https://tanstack.com/table/latest/docs/framework/react/guide/column-filtering)
> row-model stages. Its manual modes instead treat caller-provided rows as
> already sorted or filtered. A domain package might define a bounded property for one
...
| --- | --- | --- | --- | --- |
| Initialize a verified store | init | `evidence.request` | Not advertised | **Implemented** |
| Summarize store and open challenges | status | `evidence.request` | evidence_status | **Implemented** |
| Assess one exact structured claim | assess | Document diagnostics and `evidence.request` | evidence_assess | **Implemented** |
| Run the Endpoints safe-skip check | run-endpoints | `evidence.request` | evidence_run_endpoints | **Implemented, Endpoints-specific** |
| Update dependency context | context | `evidence.request` | evidence_context | **Implemented** |
| Read observations and challenge history | history | `evidence.request` | evidence_history | **Implemented** |
| Link a challenge to a replay | resolve | `evidence.request` | evidence_resolve | **Implemented** |
...
behavior, not universal consumer policy. It also exposes the shared service
through `evidence.request`. The MCP server advertises six tools and returns both
text and structured results.

All three use the same local service and store. A shared integration test
exercises those paths. No packaged editor configuration, contextual source
diagnostic rendering, MCP installation, schema-quality pass, real agent
registration, or cross-process deployment has been demonstrated.
...
> **Illustrative TanStack Form case—not implementation evidence:** Form accepts
> [Standard Schema validators](https://tanstack.com/form/latest/docs/framework/react/guides/validation)
> and propagates schema errors to fields. Form-level validators can also return
> separate form and field errors. A domain package might define how a
> particular adapter maps a bounded class of validation results into those
> structures. The adapter version, schema library, configuration, input shape,
> and unsupported issue types could affect applicability. Trust could preserve
> observations and counterexamples; Form would define the correct mapping.
...
The evidence behind these transfers is not uniform. The list combines
conceptual analogies, source-guided design donors, direct workflow lineage,
unevaluated design transfers, supplied-source arguments, and bounded Endpoints
implementation evidence. Concretely, proof assistants are a conceptual
analogy; ATMS, Carneades, and the formal-system families are source-guided
donors; Field Lab is direct workflow lineage; shadcn and Beads are unevaluated
design transfers; bounded-control assurance is a source-guided product
argument; and Endpoints alone supplies bounded implementation evidence. None
is independent validation of Trust.
...
- **ATMS and structured argument systems such as Carneades:** alternate support,
  joint premises, and defeaters inform the evidence graph. Static argument
  structure does not supply current applicability, execution reach, or repair
  causality.
- **Datalog systems such as Soufflé:** facts, recursive rules, and proof trees
  clarify derivation. Entailment remains relative to supplied facts and rules;
  positive derivation does not itself model applicability, defeat, expiry, or
  repair.
- **SAT and SMT solvers such as Z3:** models, unsatisfiable cores, and explicit
  unknown outcomes supply useful evidence forms. The encoding remains a trust
  root, cores need not be minimal, and solver state is not durable evidence
  history.
...
- **Provenance semirings:** addition and multiplication express alternate and
  ...
  votes.
- **Evidence-oriented workflows, including Field Lab:** separating design,
  maintenance, hostile checks, repair, provenance, and human checkpoints
  clarifies work. Following a workflow does not validate the result.
- **shadcn-style lint diagnostics:** contextual findings, editable targets, and
  ...
  correct.
- **Beads-style task systems:** gaps and dependencies can become discoverable
  work for later agents. Task closure and counts do not grant support and can
  hide partial routes.
...
- Add signatures, authentication, remote authority, hostile-process isolation
  including a hostile-plugin boundary, and evidence stores controlled by a
  separate actor only under an explicit threat model.
...
- Keep independent range **unmeasured**; no second implementation is currently
  planned. If extraction is selected later, that decision may create the reason
  to test a second domain then; it is not a current milestone.
...
2. **Complete the Endpoints agent loop.**
   ...
   - Keep safe-skip bounded while expanding analyzer evidence and scoping
     dependencies by endpoint or artifact.
...
3. **Ship and evaluate the real experience.**
   ...
   - Harden multi-process coordination, authentication, hostile-process
     isolation, remote execution, and scale for the deployment Endpoints
     actually needs.
