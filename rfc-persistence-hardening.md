...
| D6  | Sync-present readiness is gated entirely on the source: if Electric neither reaches `up-to-date` nor raises `onError` (the normal offline retry case), `markReady` never fires even though local hydration completed and the rows are in the collection. Local-only mode already marks ready after hydration — the asymmetry is the defect. | #1416, #1443                    | RED test (`size === 1`, status stuck `loading`); kept as `it.fails`                        |
...
### PR 4 — review & land the four waiting community PRs
...
1. **Local-ready semantics (PR 6).** When a sync-present persisted collection
   ...
   reports (#1416, #1443) all wanted local-first.
