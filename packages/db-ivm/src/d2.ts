...
export class D2 implements ID2 {
  ...
  step(): void {
    ...
    console.debug(`[TanStack-DB-DEBUG] D2.step: running ${this.#operators.length} operators`, {
      operators: this.#operators.map((op) => ({ id: op.id, type: op.operatorType })),
    })
    for (const op of this.#operators) {
      const hadWork = op.hasPendingWork()
      if (hadWork) {
        console.debug(`[TanStack-DB-DEBUG] D2.step: operator ${op.id} (${op.operatorType}) has pending work, running`)
      }
      op.run()
    }
  }
