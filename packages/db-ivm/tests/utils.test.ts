...
describe(`hash`, () => {
  ...
  describe(`object types`, () => {
    ...
    it(`should hash Buffers, Uint8Arrays and File objects by reference`, () => {
      ...
      expect(typeof hash1).toBe(hashType)
      expect(hash1).not.toBe(hash2) // Same content but different instances have a different hash because it would be too costly to deeply hash buffers
      ...
      expect(typeof hash4).toBe(hashType)
      expect(hash4).not.toBe(hash5) // Same content but different instances have a different hash
