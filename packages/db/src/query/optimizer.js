...
import { CollectionRef as CollectionRefClass, Func, QueryRef as QueryRefClass, } from "./ir.js";
/**
...
function applyRecursiveOptimization(query) {
    ...
    const subqueriesOptimized = {
        ...query,
        from: query.from.type === `queryRef`
            ? new QueryRefClass(applyRecursiveOptimization(query.from.query), query.from.alias)
            : query.from,
        join: query.join?.map((joinClause) => ({
            ...joinClause,
            from: joinClause.from.type === `queryRef`
                ? new QueryRefClass(applyRecursiveOptimization(joinClause.from.query), joinClause.from.alias)
                : joinClause.from,
        })),
    };
...
function splitAndClauses(whereClauses) {
    ...
    for (const clause of whereClauses) {
        if (clause.type === `func` && clause.name === `and`) {
            ...
            const splitArgs = splitAndClauses(clause.args);
...
function groupWhereClauses(analyzedClauses) {
    ...
    const combinedMultiSource = multiSource.length > 0 ? combineWithAnd(multiSource) : undefined;
    return {
...
function applyOptimizations(query, groupedClauses) {
    ...
    const optimizedFrom = optimizeFromWithTracking(query.from, groupedClauses.singleSource, actuallyOptimized);
    // Optimize JOIN clauses and track what was optimized
    const optimizedJoins = query.join
        ? query.join.map((joinClause) => ({
            ...joinClause,
            from: optimizeFromWithTracking(joinClause.from, groupedClauses.singleSource, actuallyOptimized),
        }))
        : undefined;
...
function deepCopyQuery(query) {
    return {
        ...
        from: query.from.type === `collectionRef`
        ...
        join: query.join
            ? query.join.map((joinClause) => ({
                ...
                from: joinClause.from.type === `collectionRef`
                    ? new CollectionRefClass(joinClause.from.collection, joinClause.from.alias)
                    : new QueryRefClass(deepCopyQuery(joinClause.from.query), joinClause.from.alias),
            }))
            : undefined,
...
function isSafeToPushIntoExistingSubquery(query) {
    ...
    if (query.select) {
        const hasAggregates = Object.values(query.select).some((expr) => expr.type === `agg`);
        if (hasAggregates) {
            return false;
        }
    }
