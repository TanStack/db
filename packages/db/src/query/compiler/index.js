 ...
 */
export function compileQuery(rawQuery, inputs, cache = new WeakMap(), queryMapping = new WeakMap()) {
    ...
    const { alias: mainTableAlias, input: mainInput } = processFrom(query.from, allInputs, cache, queryMapping);
    tables[mainTableAlias] = mainInput;
    // Prepare the initial pipeline with the main table wrapped in its alias
    let pipeline = mainInput.pipe(map(([key, row]) => {
    ...
    if (query.join && query.join.length > 0) {
        pipeline = processJoins(pipeline, query.join, tables, mainTableAlias, allInputs, cache, queryMapping);
    }
    ...
    if (query.fnSelect) {
        ...
        pipeline = pipeline.pipe(map(([key, namespacedRow]) => {
            const selectResults = !query.join && !query.groupBy
    ...
    if (query.groupBy && query.groupBy.length > 0) {
        pipeline = processGroupBy(pipeline, query.groupBy, query.having, query.select, query.fnHaving);
    }
    else if (query.select) {
        ...
        if (hasAggregates) {
            ...
            pipeline = processGroupBy(pipeline, [], // Empty group by means single group
            query.having, query.select, query.fnHaving);
        }
    }
    ...
    if (query.orderBy && query.orderBy.length > 0) {
        const orderedPipeline = processOrderBy(pipeline, query.orderBy, query.limit, query.offset);
        // Final step: extract the __select_results and include orderBy index
        const resultPipeline = orderedPipeline.pipe(map(([key, [row, orderByIndex]]) => {
    ...
    const resultPipeline = pipeline.pipe(map(([key, row]) => {
        ...
        const finalResults = row.__select_results;
        return [key, [finalResults, undefined]];
    }));
    const result = resultPipeline;
    // Cache the result before returning (use original query as key)
    cache.set(rawQuery, result);
...
function mapNestedQueries(optimizedQuery, originalQuery, queryMapping) {
    ...
    if (optimizedQuery.from.type === `queryRef` &&
        ...
        mapNestedQueries(optimizedQuery.from.query, originalQuery.from.query, queryMapping);
    }
    ...
    if (optimizedQuery.join && originalQuery.join) {
        for (let i = 0; i < optimizedQuery.join.length && i < originalQuery.join.length; i++) {
            ...
            if (optimizedJoin.from.type === `queryRef` &&
