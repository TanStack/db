 ...
 */
export function processSelectToResults(pipeline, select, _allInputs) {
    ...
    return pipeline.pipe(map(([key, namespacedRow]) => {
        const selectResults = {};
 ...
 */
export function processSelect(pipeline, select, _allInputs) {
    // Pre-compile all select expressions
    const compiledSelect = [];
    const spreadAliases = [];
    for (const [alias, expression] of Object.entries(select)) {
        if (alias.startsWith(`__SPREAD_SENTINEL__`)) {
            ...
            if (isAggregateExpression(expression)) {
                ...
                throw new Error(`Aggregate expressions in SELECT clause should be handled by GROUP BY processing`);
            }
            compiledSelect.push({
                alias,
                compiledExpression: compileExpression(expression),
 ...
 */
export function processArgument(arg, namespacedRow) {
    if (isAggregateExpression(arg)) {
        throw new Error(`Aggregate expressions are not supported in this context. Use GROUP BY clause for aggregates.`);
    }
