...
function validateAndCreateMapping(groupByClause, selectClause) {
    const selectToGroupByIndex = new Map();
    ...
    for (const [alias, expr] of Object.entries(selectClause)) {
        ...
        if (groupIndex === -1) {
            throw new Error(`Non-aggregate expression '${alias}' in SELECT must also appear in GROUP BY clause`);
        }
        // Cache the mapping
 ...
 */
export function processGroupBy(pipeline, groupByClause, havingClauses, selectClause, fnHavingClauses) {
    ...
    if (groupByClause.length === 0) {
        ...
        if (havingClauses && havingClauses.length > 0) {
            for (const havingClause of havingClauses) {
                const transformedHavingClause = transformHavingClause(havingClause, selectClause || {});
...
function expressionsEqual(expr1, expr2) {
    ...
    switch (expr1.type) {
        case `ref`:
            ...
            if (!expr1.path || !expr2.path)
                return false;
            if (expr1.path.length !== expr2.path.length)
                return false;
        ...
        case `func`:
            return (expr1.name === expr2.name &&
                ...
                (expr1.args || []).every((arg, i) => expressionsEqual(arg, expr2.args[i])));
        case `agg`:
            return (expr1.name === expr2.name &&
                expr1.args?.length === expr2.args?.length &&
                (expr1.args || []).every((arg, i) => expressionsEqual(arg, expr2.args[i])));
...
function transformHavingClause(havingExpr, selectClause) {
    switch (havingExpr.type) {
        case `agg`: {
            ...
            throw new Error(`Aggregate function in HAVING clause must also be in SELECT clause: ${aggExpr.name}`);
        }
        case `func`: {
            const funcExpr = havingExpr;
            // Transform function arguments recursively
            const transformedArgs = funcExpr.args.map((arg) => transformHavingClause(arg, selectClause));
        ...
        case `val`:
            ...
            return havingExpr;
        default:
            throw new Error(`Unknown expression type in HAVING clause: ${havingExpr.type}`);
    }
}
/**
