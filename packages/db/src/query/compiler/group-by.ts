...
export function containsAggregate(
  ...
  if (expr.type === `conditionalSelect` && `branches` in expr) {
    ...
    const defaultValue = (expr as {
      defaultValue?: BasicExpression | Aggregate | Select | { type: string }
    }).defaultValue

    return (
      branchHasAggregate ||
      (defaultValue !== undefined && containsAggregate(defaultValue))
    )
  }
  return false
}
