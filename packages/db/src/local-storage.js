...
function validateJsonSerializable(value, operation) {
    try {
        ...
        throw new Error(`Cannot ${operation} item because it cannot be JSON serialized: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 ...
 * )
 */
export function localStorageCollectionOptions(config) {
    ...
    if (!storage) {
        throw new Error(`[LocalStorageCollection] No storage available. Please provide a storage option or ensure window.localStorage is available.`);
    }
    ...
    if (!storageEventApi) {
        throw new Error(`[LocalStorageCollection] No storage event API available. Please provide a storageEventApi option or ensure window is available.`);
    }
    // Track the last known state to detect changes
    const lastKnownData = new Map();
    // Create the sync configuration
    const sync = createLocalStorageSync(config.storageKey, storage, storageEventApi, config.getKey, lastKnownData);
    /**
        ...
        if (typeof parsed === `object` &&
            ...
            Object.entries(parsed).forEach(([key, value]) => {
                ...
                if (value &&
                    ...
                    throw new Error(`[LocalStorageCollection] Invalid data format in storage key "${storageKey}" for key "${key}".`);
                }
            });
        }
        else {
            throw new Error(`[LocalStorageCollection] Invalid data format in storage key "${storageKey}". Expected object format.`);
        }
        return dataMap;
    }
    catch (error) {
...
function createLocalStorageSync(storageKey, storage, storageEventApi, _getKey, lastKnownData) {
    ...
    const syncConfig = {
        ...
        getSyncMetadata: () => ({
            storageKey,
            storageType: storage === (typeof window !== `undefined` ? window.localStorage : null)
                ? `localStorage`
                : `custom`,
        }),
