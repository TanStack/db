import type { IndexResolver, IndexConstructor } from "./base-index.js"
...
export interface IndexOptions<TResolver extends IndexResolver = IndexResolver> {
  ...
  options?: TResolver extends IndexConstructor<any>
    ? TResolver extends new (id: string, expr: any, name?: string, options?: infer O) => any ? O : never
    : TResolver extends () => Promise<infer TCtor>
      ? TCtor extends new (id: string, expr: any, name?: string, options?: infer O) => any ? O : never
      : never
}
...
export type ResolvedIndexType<TResolver extends IndexResolver> = 
  TResolver extends IndexConstructor<infer TKey>
    ? InstanceType<TResolver>
    : TResolver extends () => Promise<IndexConstructor<infer TKey>>
      ? TResolver extends () => Promise<infer TCtor>
        ? TCtor extends IndexConstructor<TKey>
          ? InstanceType<TCtor>
          : never
        : never
      : never