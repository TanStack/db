...
collection.update("theme", (draft) => { draft.mode = "light" })
...
tx.mutate(() => { localData.insert({ id: "draft-1", data: "..." }) })
await tx.commit()
```
...
const encryptedStorage = {
  getItem: (key) => { const v = localStorage.getItem(key); return v ? decrypt(v) : null },
  setItem: (key, value) => localStorage.setItem(key, encrypt(value)),
  removeItem: (key) => localStorage.removeItem(key),
}
localStorageCollectionOptions({ id: "secure", storageKey: "enc-key", storage: encryptedStorage, getKey: (i) => i.id })
```
...
const modalState = createCollection(
  localOnlyCollectionOptions({
    id: "modal-state",
    getKey: (item) => item.id,
    initialData: [{ id: "confirm-delete", isOpen: false }, { id: "settings", isOpen: false }],
  })
)
...
modalState.update("settings", (draft) => { draft.isOpen = true })
userPrefs.insert({ id: "current-user", theme: "dark", language: "en", notifications: true })
```
