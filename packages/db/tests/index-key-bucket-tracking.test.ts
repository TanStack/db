...
describe(`a key lives in at most one bucket at a time`, () => {
  it(`BasicIndex.update moves the key out of its actual bucket, regardless of the oldItem passed`, () => {
    const index = new BasicIndex<string>(1, new PropRef([`current_stage_id`]))
    index.add(`P`, row(`P`, `A`))
  ...
  it(`BasicIndex.add called twice without an intervening remove keeps the key in the latest bucket only`, () => {
    const index = new BasicIndex<string>(1, new PropRef([`current_stage_id`]))
    index.add(`P`, row(`P`, `A`))
    index.add(`P`, row(`P`, `B`))
  ...
  it(`BTreeIndex.update moves the key out of its actual bucket, regardless of the oldItem passed`, () => {
    const index = new BTreeIndex<string>(1, new PropRef([`current_stage_id`]))
    index.add(`P`, row(`P`, `A`))
  ...
  it(`BTreeIndex.add called twice without an intervening remove keeps the key in the latest bucket only`, () => {
    const index = new BTreeIndex<string>(1, new PropRef([`current_stage_id`]))
    index.add(`P`, row(`P`, `A`))
    index.add(`P`, row(`P`, `B`))
  ...
  it(`an optimistic→synced update on the indexed column ends with the row in the new bucket`, async () => {
    ...
    const idx = Array.from(collection.indexes.values())[0] as BasicIndex<string>
