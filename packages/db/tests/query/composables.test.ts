...
describe(`Composables`, () => {
  describe(`defineForRow`, () => {
    ...
    test(`should create reusable callback predicates`, () => {
      ...
      const userIsYoung = defineForRow<{ user: User }>().callback(({ user }) =>
        lt(user.age, 25)
      )
    ...
    test(`should create reusable select objects`, () => {
      ...
      const userBasicInfo = defineForRow<{ user: User }>().select(
        ({ user }) => ({
          id: user.id,
          name: user.name,
          email: user.email,
      ...
      const userNameTransforms = defineForRow<{ user: User }>().select(
        ({ user }) => ({
    ...
    test(`should work with defineQuery for reusable query composition`, () => {
      ...
      const userDisplayInfo = defineForRow<{ user: User }>().select(
        ({ user }) => ({
          userId: user.id,
      ...
      const activeAdultUsersQuery = defineQuery((q) =>
        q
          .from({ user: usersCollection })
          .where(({ user }) =>
            and(userIsAdult({ user }), eq(user.active, true))
          )
          .select(userDisplayInfo)
      )
    ...
    test(`should work with joins using defineForRow components`, () => {
      ...
      const postIsPublished = defineForRow<{ post: Post }>().callback(
        ({ post }) => eq(post.published, true)
      )

      const userPostJoinInfo = defineForRow<{
        user: User
        post: Post
      }>().select(({ user, post }) => ({
        authorId: user.id,
    ...
    test(`should work with predefined queries as subqueries using defineForRow`, () => {
      ...
      const userIsJunior = defineForRow<{ user: User }>().callback(({ user }) =>
        lte(user.age, 25)
      )

      const userBasicWithAge = defineForRow<{ user: User }>().select(
        ({ user }) => ({
          id: user.id,
          name: user.name,
          age: user.age,
      ...
      const activeJuniorUsersQuery = defineQuery((q) =>
        q
          .from({ user: usersCollection })
          .where(userIsActive)
          .where(userIsJunior)
      ...
      const enhancedJuniorUsersQuery = defineQuery((q) =>
        q
    ...
    test(`should maintain type safety across different namespace structures`, () => {
      ...
      const singleUserPredicate = defineForRow<{ user: User }>().callback(
        ({ user }) => gt(user.age, 20)
      )

      const joinedUserPostPredicate = defineForRow<{
        u: User
        p: Post
      }>().callback(({ u, p }) =>
      ...
      const singleUserSelect = defineForRow<{ user: User }>().select(
        ({ user }) => ({
          name: user.name,
          age: user.age,
        })
      ...
      const joinedSelect = defineForRow<{ u: User; p: Post }>().select(
        ({ u, p }) => ({
          userName: u.name,
          postTitle: p.title,
        })
