...
it(`replays one unindexed fallback and publishes one replacement after truncate`, async () => {
  ...
  try {
    ...
    const messagesBeforePrivateReplay = builder.currentSyncState!.messagesCount
    ...
    expect(builder.currentSyncState!.messagesCount).toBeGreaterThan(
      messagesBeforePrivateReplay,
    )
    expect(readRows()).toEqual([{ id: `a`, rank: 1 }])
    expect(batches).toHaveLength(0)
    expect(callbackReads).toHaveLength(0)
...
it(`holds root and collection-valued include publication until replay succeeds`, async () => {
  type Parent = { id: string; groupId: string; rank: number }
  type Child = { id: string; groupId: string; value: string }
  type Result = {
    hasMore: boolean
    appliedRowKeys: ReadonlyArray<string>
  }
  const pending: Array<ReturnType<typeof createDeferred<Result>>> = []
  let begin!: () => void
  let write!: (message: { type: `insert`; value: Parent }) => void
  let commit!: () => true | Promise<void>
  let truncate!: () => void
  const parent = createCollection<Parent>({
    id: `replay-publication-gate-parent`,
    getKey: (row) => row.id,
    syncMode: `on-demand`,
    startSync: true,
    autoIndex: `off`,
  ...
  const childOptions = mockSyncCollectionOptions<Child>({
    id: `replay-publication-gate-child`,
    getKey: (row) => row.id,
    initialData: [
      { id: `c1`, groupId: `g1`, value: `old-one` },
      { id: `c2`, groupId: `g2`, value: `old-two` },
    ],
    autoIndex: `eager`,
  })
  const child = createCollection(childOptions)
  const live = createLiveQueryCollection({
    id: `replay-publication-gate-live`,
    query: (q) =>
      q
        .from({ parent })
        .orderBy(({ parent: row }) => row.rank)
        .limit(1)
        .select(({ parent: row }) => ({
          id: row.id,
          groupId: row.groupId,
          children: q
            .from({ child })
            .where(({ child: childRow }) => eq(childRow.groupId, row.groupId))
            .select(({ child: childRow }) => ({
              id: childRow.id,
  ...
  const readRoot = () =>
    live.toArray.map((row) => ({
      id: row.id,
      groupId: row.groupId,
      children: row.children.toArray.map(({ id, value }) => ({ id, value })),
    }))
  const rootCallbackReads: Array<ReturnType<typeof readRoot>> = []
  const rootBatches: Array<number> = []
  const rootObserver = live.subscribeChanges(
    (changes) => {
      rootBatches.push(changes.length)
      rootCallbackReads.push(readRoot())
    },
    { includeInitialState: false },
  )
  const preload = live.preload()
  ...
  try {
    begin()
    write({ type: `insert`, value: { id: `p`, groupId: `g1`, rank: 1 } })
    const initialApplied = commit()
    if (initialApplied !== true) await initialApplied
    pending[0]!.resolve({ hasMore: false, appliedRowKeys: [`p`] })
    await preload
    await flushPromises()
    ...
    expect(readRoot()).toEqual([
      {
        id: `p`,
        groupId: `g1`,
        children: [{ id: `c1`, value: `old-one` }],
      },
    ])
    const oldFacade = live.toArray[0]!.children
    const oldFacadeBatches: Array<number> = []
    const oldFacadeReads: Array<ReadonlyArray<string>> = []
    const oldFacadeObserver = oldFacade.subscribeChanges(
      (changes) => {
        oldFacadeBatches.push(changes.length)
        oldFacadeReads.push(oldFacade.toArray.map(({ id }) => id))
      },
      { includeInitialState: false },
    )

    rootBatches.length = 0
    rootCallbackReads.length = 0
    const replacement = (() => {
      begin()
      truncate()
      return commit()
    ...
    begin()
    write({ type: `insert`, value: { id: `p`, groupId: `g2`, rank: 1 } })
    const replacementApplied = commit()
    if (replacementApplied !== true) await replacementApplied
    childOptions.utils.begin()
    childOptions.utils.write({
      type: `update`,
      value: { id: `c2`, groupId: `g2`, value: `new-two` },
    })
    childOptions.utils.commit()
    await flushPromises()
    ...
    expect(oldFacade.toArray.map(({ id, value }) => ({ id, value }))).toEqual([
      { id: `c1`, value: `old-one` },
    ])
    expect(rootBatches).toEqual([])
    expect(oldFacadeBatches).toEqual([])

    pending[1]!.resolve({ hasMore: false, appliedRowKeys: [`p`] })
    if (replacement !== true) await replacement
    await flushPromises()
    ...
    expect(readRoot()).toEqual([
      {
        id: `p`,
        groupId: `g2`,
        children: [{ id: `c2`, value: `new-two` }],
      },
    ])
    expect(oldFacade.toArray).toEqual([])
    expect(rootBatches).toEqual([1])
    expect(rootCallbackReads).toEqual([readRoot()])
    expect(oldFacadeBatches).toEqual([1])
    expect(oldFacadeReads).toEqual([[]])
    oldFacadeObserver.unsubscribe()
  } finally {
    for (const request of pending) {
      request.reject(new Error(`test cleanup`))
    }
    rootObserver.unsubscribe()
    await Promise.all([
      preload.catch(() => undefined),
      live.cleanup(),
      parent.cleanup(),
...
it(`waits for every recovering source before publishing a joined replacement`, async () => {
  type Primary = { id: string; joinKey: string; rank: number }
  type Secondary = { id: string; joinKey: string; label: string }
  type Result = {
    hasMore: boolean
    appliedRowKeys: ReadonlyArray<string>
  }
  const createSource = <Row extends { id: string }>(
    id: string,
    autoIndex: `off` | `eager`,
  ) => {
    const pending: Array<ReturnType<typeof createDeferred<Result>>> = []
  ...
  const primary = createSource<Primary>(`joined-recovery-primary`, `off`)
  const secondary = createSource<Secondary>(
    `joined-recovery-secondary`,
    `eager`,
  )
  const live = createLiveQueryCollection({
    id: `joined-recovery-live`,
    query: (q) =>
      q
  ...
  const readRows = () =>
    live.toArray.map(({ id, rank, label }) => ({ id, rank, label }))
  const batches: Array<number> = []
  const callbackReads: Array<ReturnType<typeof readRows>> = []
  const observer = live.subscribeChanges(
    (changes) => {
  ...
  try {
    ...
    await primary.apply({ id: `p`, joinKey: `shared`, rank: 1 })
    ...
    await secondary.apply({ id: `s`, joinKey: `shared`, label: `old` })
    secondary.pending[0]!.resolve({
      hasMore: false,
      appliedRowKeys: [`s`],
    })
    expect(primary.pending).toHaveLength(2)
    primary.pending[1]!.resolve({
      hasMore: false,
      appliedRowKeys: [`p`],
    })
    await preload
    await flushPromises()
    expect(readRows()).toEqual([{ id: `p`, rank: 1, label: `old` }])

    batches.length = 0
    callbackReads.length = 0
    const initialPrimaryLoads = primary.pending.length
    const initialSecondaryLoads = secondary.pending.length
    const secondaryReplay = secondary.startReplay()
    const primaryReplay = primary.startReplay()
    await flushPromises()
    ...
    await primary.apply({ id: `p`, joinKey: `shared`, rank: 2 })
    await secondary.apply({ id: `s`, joinKey: `shared`, label: `new` })
    expect(readRows()).toEqual([{ id: `p`, rank: 1, label: `old` }])
    expect(batches).toEqual([])

    for (const request of primary.pending.slice(initialPrimaryLoads)) {
      request.resolve({ hasMore: false, appliedRowKeys: [`p`] })
    }
    if (primaryReplay !== true) await primaryReplay
    await flushPromises()
    expect(readRows()).toEqual([{ id: `p`, rank: 1, label: `old` }])
    expect(batches).toEqual([])

    for (const request of secondary.pending.slice(initialSecondaryLoads)) {
      request.resolve({ hasMore: false, appliedRowKeys: [`s`] })
    }
    if (secondaryReplay !== true) await secondaryReplay
    await flushPromises()

    expect(readRows()).toEqual([{ id: `p`, rank: 2, label: `new` }])
    expect(batches).toEqual([1])
    expect(callbackReads).toEqual([[{ id: `p`, rank: 2, label: `new` }]])
  } finally {
...
it(`keeps an optimistic overlay above a replay replacement`, async () => {
  const harness = createUnindexedReplayHarness(
    `unindexed-replay-optimistic-overlay`,
  )
  const preload = harness.live.preload()
  const persistence = createDeferred<void>()
  const rollback = new Error(`optimistic update rolled back`)
  const updateRank = createOptimisticAction<number>({
    onMutate: (rank) => {
      harness.live.update(`a`, (draft) => {
        draft.rank = rank
      })
    },
    mutationFn: () => persistence.promise,
  })
  let transaction: ReturnType<typeof updateRank> | undefined
  ...
  try {
    ...
    transaction = updateRank(10)
    expect(harness.readRows()).toEqual([{ id: `a`, rank: 10 }])

    harness.clearObservations()
    const replacement = harness.startTruncate()
    await flushPromises()
    await harness.applyRows([{ id: `a`, rank: 2 }])
    expect(harness.readRows()).toEqual([{ id: `a`, rank: 10 }])
    ...
    persistence.reject(rollback)
    await expect(transaction.isPersisted.promise).rejects.toBe(rollback)
    expect(harness.readRows()).toEqual([{ id: `a`, rank: 2 }])
    expect(harness.live.get(`a`)!.$synced).toBe(true)
  } finally {
    persistence.reject(new Error(`test cleanup`))
    await Promise.all([
      preload.catch(() => undefined),
...
it(`discards private replay output when its live-query session is cleaned`, async () => {
  const harness = createUnindexedReplayHarness(
    `unindexed-private-replay-cleanup`,
  )
  const preload = harness.live.preload()
  ...
  try {
    ...
    await harness.applyRowsForRequest(1, [{ id: `b`, rank: 2 }])
    expect(harness.readRows()).toEqual([{ id: `a`, rank: 1 }])
    expect(harness.batches).toEqual([])
    ...
    await Promise.all([
      preload.catch(() => undefined),
      Promise.resolve(replacement).catch(() => undefined),
      harness.cleanup(),
    ])
  }
})
...
it.each([`async`, `sync`] as const)(
  ...
  async (successMode) => {
    ...
    try {
      ...
      const builder = harness.live.utils[LIVE_QUERY_INTERNAL].getBuilder()
      const messagesBeforePrivateProgress =
        builder.currentSyncState!.messagesCount
      await harness.applyRows([{ id: `private`, rank: 0 }])
      expect(builder.currentSyncState!.messagesCount).toBeGreaterThan(
        messagesBeforePrivateProgress,
      )
      expect(harness.readRows()).toEqual([{ id: `a`, rank: 1 }])
      expect(harness.batches).toEqual([])
      expect(harness.callbackReads).toEqual([])
