...
describe(`BTreeIndex - Issue #1186: Infinite loop with undefined indexed values`, () => {
    ...
    collection = createCollection<TaskItem, string>({
      ...
      sync: {
        sync: ({ begin, write, commit, markReady }) => {
          beginFn = begin
          writeFn = write as (mutation: { type: string; value: TaskItem }) => void
          commitFn = commit
  ...
  it(`should not infinite loop when take() is called on index with undefined values`, { timeout: 5000 }, async () => {
  ...
  it(`should not infinite loop when takeReversed() is called on index with undefined values`, { timeout: 5000 }, async () => {
    const index = collection.createIndex((row) => row.taskDate)
  ...
  it(`should correctly return items when mixing undefined and defined indexed values`, { timeout: 5000 }, async () => {
    const index = collection.createIndex((row) => row.taskDate)
  ...
  it(`should handle take() with limit when only undefined values exist`, { timeout: 5000 }, async () => {
  ...
  it(`should handle the exact scenario from issue #1186 - live query with orderBy and limit`, { timeout: 5000 }, async () => {
  ...
  it(`should handle live query with where, orderBy and limit with undefined values`, { timeout: 5000 }, async () => {
    // Create index on taskDate field
    collection.createIndex((row) => row.taskDate)
