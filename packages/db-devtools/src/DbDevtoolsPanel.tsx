...
export function DbDevtoolsPanel(props: DbDevtoolsPanelProps) {
  ...
  return (
    <div style={panelStyle} onClick={(e) => e.target === e.currentTarget && props.onClose()}>
...
function TransactionDetails(props: { transactionId: string, registry: DbDevtoolsRegistry }) {
        ...
        {(tx) => (
          <div>
            ...
            <div style={{ 'margin-bottom': '20px' }}>
              <div><strong>Collection:</strong> {tx().collectionId}</div>
              <div><strong>State:</strong> {tx().state}</div>
              <div><strong>Created:</strong> {tx().createdAt.toLocaleString()}</div>
              <div><strong>Updated:</strong> {tx().updatedAt.toLocaleString()}</div>
              <div><strong>Persisted:</strong> {tx().isPersisted ? 'Yes' : 'No'}</div>
            </div>
