...
/**
 * Create a live query with Preact Suspense support
...
export function useLiveSuspenseQuery(
  ...
  if (result.status === `loading` || result.status === `idle`) {
    ...
    // THROW PROMISE - Preact Suspense catches this (Preact 10.8+ required)
    // Note: We don't check Preact version here. In Preact <10.8, this will be caught
