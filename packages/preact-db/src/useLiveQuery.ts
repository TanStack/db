...
export function useLiveQuery(
  ...
  // Use state to trigger re-renders on collection changes
  const [version, setVersion] = useState(0)

  // Use refs to track memoized snapshot
  ...
  if (needsNewCollection) {
    if (isCollection) {
      // It's already a collection, ensure sync is started for Preact hooks
  ...
  // Reset version when collection changes
  if (needsNewCollection) {
    setVersion(0)
  ...
  // Track last snapshot and the returned value separately
