...
description: >
  ...
  isReady, isError. Import from @tanstack/octane-db (re-exports all of
  @tanstack/db).
type: framework
library: db
framework: octane
...
```tsx
import { useLiveQuery, eq, not } from '@tanstack/octane-db'
...
`@tanstack/octane-db` re-exports everything from `@tanstack/db`. In Octane projects, import everything from `@tanstack/octane-db`.
...
```tsx
import { usePacedMutations, debounceStrategy } from "@tanstack/octane-db"
...
```tsx
import { toArray, eq } from '@tanstack/octane-db'
