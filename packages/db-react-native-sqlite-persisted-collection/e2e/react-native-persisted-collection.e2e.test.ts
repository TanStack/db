...
type ReactNativePersistedCollectionTestConfig = E2ETestConfig
...
let config: ReactNativePersistedCollectionTestConfig | undefined
...
function createPersistedCollection<T extends PersistableRow>(
  ...
  const persistence = createReactNativeSQLitePersistence<T, string | number>({
    driver,
  })
...
function getConfig(): Promise<ReactNativePersistedCollectionTestConfig> {
  if (!config) {
    throw new Error(`React Native conformance config is not initialized`)
  }
