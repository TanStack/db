  ...
  describe(`Query Garbage Collection`, () => {
    ...
    it(`should only delete non-shared rows when one of multiple overlapping queries is GCed`, async () => {
      ...
      const queryFn = vi.fn().mockImplementation((context) => {
        const { meta } = context
        const tsDb = meta?.tsDb ?? {}
        const { where, orderBy, limit } = tsDb
      ...
      // Wait for initial data to load (query 1)
      await vi.waitFor(() => {
        expect(collection.size).toBe(3)
      })

      // Add query 2 with different predicates
      ...
      await collection.syncMore({ where: { category: `B` } } as any)
      ...
      await collection.syncMore({ where: { category: `C` } } as any)
    ...
    it(`should handle GC of queries with identical data`, async () => {
      ...
      await collection.syncMore({ where: { category: `A` } } as any)
    ...
    it(`should handle GC of empty queries gracefully`, async () => {
      ...
      // Wait for initial data to load (query 1 - empty)
      await vi.waitFor(() => {
    ...
    it(`should handle concurrent GC of multiple queries`, async () => {
      ...
      // Add query 2 with different predicates
      await collection.syncMore({ where: { type: `A` } } as any)
      ...
      // Add query 3 with different predicates
      await collection.syncMore({ where: { type: `B` } } as any)
