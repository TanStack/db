...
export function ReactDbDevtools(props: ReactDbDevtoolsProps = {}): React.ReactElement | null {
...
export function ReactDbDevtoolsPanel(props: ReactDbDevtoolsProps = {}): React.ReactElement | null {
  const ref = React.useRef<HTMLDivElement>(null)
  ...
  React.useEffect(() => {
    if (ref.current) {
      ...
      import('@tanstack/db-devtools').then(({ DbDevtoolsPanel }) => {
        import('solid-js/web').then(({ render }) => {
          import('@tanstack/db-devtools').then(({ initializeDevtoolsRegistry }) => {
