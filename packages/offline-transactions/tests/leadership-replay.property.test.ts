 ...
 * stale reads. Regaining leadership may replay durable pending rows, but an
 ...
 *
 * A fake durable outbox, explicit leadership callbacks, and gated provider and
 * delete acknowledgements form the history grammar. `foldReplayLedger` models
 * stored and durably removed IDs without production revision counters.
 * Generated histories hold a real storage-read delivery while removing a
 * prefix. The real OfflineExecutor, TransactionExecutor, scheduler, outbox, and
 * transaction path run over FakeStorageAdapter. Named checkpoints compare
 * delivered IDs, provider calls and idempotency keys, durable outbox state,
 * scheduler state, leadership, restoration, and cleanup.
 *
